...
from backend.ml.weekly_model import train_weekly_random_forest
...
def run_pipeline(ticker: str):
    ...
    # ------------------------------------------
    # 2. Sentiment Analysis (RoBERTa + Cache)
    # ------------------------------------------
    ...
    # ------------------------------------------
    # 4. Fetch Stock Data (Business-day aligned)
    # ------------------------------------------
    ...
    # ------------------------------------------
    # 7. Summary (Ollama-based LLM)
    # ------------------------------------------
    try:
        prompt = build_summary_context(merged, ticker)
    ...
    logging.info(f"Daily pipeline for {ticker} completed. Rows: {len(merged)}")
    ...
    logging.info(f"Weekly rows: {len(weekly)}")

    # ------------------------------------------
    # 10. Weekly Sentiment-Based Prediction (RandomForest)
    # ------------------------------------------
    try:
        model, weekly_pred, features = train_weekly_random_forest(weekly)

        weekly_pred.to_csv(OUTPUT_DIR / f"{base}_weekly.csv", index=False)

        logging.info(f"Weekly predictions saved for {ticker} (features: {features}).")
    except Exception as e:
        logging.error(f"Weekly model error: {e}")
